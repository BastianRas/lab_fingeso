...
// --- DATOS SIMULADOS PARA MALLA CURRICULAR (LIMPIA) ---
const mallaInformatica = ref([
  {
    semestre: 1,
    ramos: [
      { codigo: "CBM-1000", nombre: "Álgebra I" },
      { codigo: "CBM-1001", nombre: "Cálculo I" },
      { codigo: "INF-1000", nombre: "Intr. a la Ingeniería" }
    ]
  },
  {
    semestre: 2,
    ramos: [
      { codigo: "CBM-1002", nombre: "Álgebra II" },
      { codigo: "CBM-1003", nombre: "Cálculo II" },
      { codigo: "INF-1100", nombre: "Programación Básica" }
    ]
  },
  {
    semestre: 3,
    ramos: [
      { codigo: "FIS-1000", nombre: "Física I" },
      { codigo: "INF-2200", nombre: "Estructuras de Datos" },
      { codigo: "INF-2201", nombre: "Arquitectura de Computadores" }
    ]
  },
  {
    semestre: 4,
    ramos: [
      { codigo: "INF-3321", nombre: "Sistemas Operativos" },
      { codigo: "INF-3410", nombre: "Redes de Computadores" },
      { codigo: "INF-3454", nombre: "Fundamentos de Ing. de Software" }
    ]
  }
]);

// --- LÓGICA DEL HORARIO EN BLOQUES ---
const diasSemana = [
  { id: 'L', nombre: 'Lunes' },
  { id: 'M', nombre: 'Martes' },
  { id: 'W', nombre: 'Miércoles' },
  { id: 'J', nombre: 'Jueves' },
  { id: 'V', nombre: 'Viernes' }
];

const bloquesHorarios = [
  { id: 1, hora: "08:15 - 09:35" },
  { id: 2, hora: "09:50 - 11:10" },
  { id: 3, hora: "11:25 - 12:45" },
  { id: 4, hora: "13:45 - 15:05" },
  { id: 5, hora: "15:20 - 16:40" },
  { id: 6, hora: "16:55 - 18:15" },
  { id: 7, hora: "18:45 - 20:05" },
  { id: 8, hora: "20:05 - 21:25" },
  { id: 9, hora: "21:25 - 22:45" }
];
...
const horarioGenerado = computed(() => {
  ...
  // Llenar cuadrícula con las clases reales
  if (matricula.value && matricula.value.clases) {
    matricula.value.clases.forEach(clase => {
      if(clase.horario) {
        // Separa el texto "L2 W2" en ["L2", "W2"]
        const modulos = clase.horario.trim().split(/\s+/);
        modulos.forEach(mod => {
          const dia = mod.charAt(0).toUpperCase(); // L, M, W, J, V
          const bloque = parseInt(mod.substring(1)); // 2
          
          if (!isNaN(bloque) && grid[bloque] && grid[bloque][dia] !== undefined) {
            grid[bloque][dia] = clase;
          }
        });
      }
    });
  ...
  <div class="dashboard-container">
    ...
    <section class="quick-actions">
      ...
      <div v-if="vistaActiva === 'horario'" class="info-panel panel-animado">
        ...
        <div class="tabla-horario-wrapper">
          <table class="tabla-horario">
            <thead>
              <tr>
                <th class="col-hora">Módulo</th>
                <th v-for="dia in diasSemana" :key="dia.id">{{ dia.nombre }}</th>
              </tr>
            </thead>
            <tbody>
              <template v-for="bloque in bloquesHorarios" :key="bloque.id">
                ...
                <tr>
                  <td class="col-hora">
                    <strong>{{ bloque.id }}</strong>
                    <div class="hora-detalle">{{ bloque.hora }}</div>
                  </td>
                  
                  <td v-for="dia in diasSemana" :key="dia.id" class="celda-clase">
                    <div v-if="horarioGenerado[bloque.id][dia.id]" class="clase-bloque">
                      <div class="clase-nombre" :title="horarioGenerado[bloque.id][dia.id].nombre">
                        {{ horarioGenerado[bloque.id][dia.id].nombre }}
                      </div>
                ...
                <tr v-if="bloque.id === 3" class="fila-descanso">
                  <td colspan="6">🍽️ Horario de Almuerzo (12:45 - 13:45)</td>
                </tr>
                <tr v-if="bloque.id === 6" class="fila-descanso">
                  <td colspan="6">☕ Descanso / Break (18:15 - 18:45)</td>
                </tr>
      ...
      <div v-if="vistaActiva === 'malla'" class="info-panel panel-animado">
        ...
        <p class="facultad">Ingeniería en Informática — Plan Vigente</p>
...
.celda-clase { vertical-align: middle; height: 60px; padding: 5px; }
...
.clase-nombre { font-size: 0.8rem; font-weight: bold; color: #1565c0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
...
/* NUEVA Malla Curricular Grid (Sin créditos) */
