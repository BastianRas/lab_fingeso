...
const cargarListaAsistencia = () => {
  if (cursoAsistenciaId.value) {
    ...
    const alumnosDelRamo = alumnosInscritos.value.filter(alumno => {
      const m = matriculasAdmin[alumno.numeroCredencial];
      if (m && m.cursos) return m.cursos.some(c => c.id === cursoAsistenciaId.value);
      return true; 
    });

    listaAsistencia.value = alumnosDelRamo.map(a => ({ ...a, presente: true }));
  }
};
...
const irAlMapaParaLugar = (ubicacion) => { alert(`📍 Calculando ruta en el campus hacia la sala: ${ubicacion}`); router.push('/mapa'); };

const alumnosParaEvaluar = computed(() => {
  if (!formNota.value.claseId) return [];
  const matriculasAdmin = JSON.parse(localStorage.getItem('matriculas_admin') || '{}');
  
  return alumnosInscritos.value.filter(alumno => {
    const m = matriculasAdmin[alumno.numeroCredencial];
    if (m && m.cursos) return m.cursos.some(c => c.id === formNota.value.claseId);
    return true; 
  });
});
  ...
  if (user && user.rol === 'PROFESOR') {
    ...
    try {
      ...
      if (misCursosAsignados && misCursosAsignados.length > 0) {
        clases.value = misCursosAsignados.map(c => ({ ...c, semestre: "1-2026" }));
      } else {
...
const enviarNota = async () => {
  ...
  if (!formNota.value.claseId || !formNota.value.alumnoId || !formNota.value.nombreEvaluacion || !formNota.value.valor || !formNota.value.ponderacion) {
    mensaje.value = { texto: '⚠️ Completa todos los campos.', tipo: 'error' }; return;
  }
...
const eliminarNota = async (id) => {
  if(confirm("¿Estás seguro de eliminar esta calificación?")) {
    if(String(id).startsWith('mock-')) {
       let notasMock = JSON.parse(localStorage.getItem('notas_mock') || '[]');
...
<template>
  <div class="dashboard-container">
    <header class="user-header">
      <div class="welcome-text"><h2>Hola, Profesor {{ profesorNombre }} 👋</h2><p>Panel Docente y Gestión de Cursos</p></div>
      <div class="header-actions"><button class="btn-logout" @click="router.push('/')">Cerrar Sesión</button><div class="avatar">👨‍🏫</div></div>
    </header>

    <section v-if="primerCurso()" class="urgent-card">
      <div class="card-header"><span class="badge">Semestre {{ primerCurso().semestre }}</span><h3>Próxima Clase: {{ primerCurso().nombre }}</h3></div>
      <div class="card-body">
        <div class="info-row"><i class="fas fa-map-marker-alt"></i><span class="highlight-location">Sala: {{ primerCurso().sala }}</span></div>
        <div class="info-row"><i class="far fa-clock"></i><span>Horario: {{ primerCurso().horario }}</span></div>
      </div>
      <div class="btn-group-urgent">
        <button class="primary-btn map-btn" @click="irAlMapaParaLugar(primerCurso().sala)">🗺️ Trazar ruta a la sala</button>
        <button class="primary-btn" @click="mostrar('asistencia')">📋 Pasar Asistencia</button>
      </div>
    </section>
    ...
    <section class="quick-actions">
      ...
      <div class="grid-buttons">
        ...
        <button class="action-btn" :class="{ active: vistaActiva === 'cursos' }" @click="mostrar('cursos')"><span class="icon">📚</span> <span class="text">Mis Cursos</span></button>
        <button class="action-btn" :class="{ active: vistaActiva === 'asistencia' }" @click="mostrar('asistencia')"><span class="icon">✅</span> <span class="text">Asistencia</span></button>
        <button class="action-btn" :class="{ active: vistaActiva === 'notas' }" @click="mostrar('notas')"><span class="icon">✍️</span> <span class="text">Subir Notas</span></button>
        <button class="action-btn" @click="router.push('/mapa')"><span class="icon">📍</span> <span class="text">Mapa Campus</span></button>
      </div>
      ...
      <div v-if="vistaActiva === 'horario'" class="info-panel animacion-panel">
        ...
        <div class="tabla-horario-wrapper">
          <table class="tabla-horario">
            <thead><tr><th class="col-hora">Módulo</th><th v-for="dia in diasSemana" :key="dia.id">{{ dia.nombre }}</th></tr></thead>
            <tbody>
      ...
      <div v-if="vistaActiva === 'cursos'" class="info-panel animacion-panel">
        ...
        <div class="cursos-grid">
          <div v-for="clase in clases" :key="clase.id" class="curso-card">
            <div class="curso-header"><span class="codigo-badge">{{ clase.codigo }}</span><span class="semestre-txt">{{ clase.semestre }}</span></div>
            <h5>{{ clase.nombre }}</h5>
      ...
      <div v-if="vistaActiva === 'asistencia'" class="info-panel animacion-panel">
        ...
        <div v-if="listaAsistencia.length > 0" class="tabla-responsive asistencia-table-box">
          <table class="data-table">
            <thead>
              <tr><th>RUT / Credencial</th><th>Nombre del Alumno</th><th class="text-center">Estado (Presente)</th></tr>
            </thead>
            <tbody>
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <select v-model="formNota.alumnoId" required>
                <option value="" disabled>Seleccione un estudiante inscrito...</option>
                <option v-for="alumno in alumnosParaEvaluar" :key="alumno.usuarioId" :value="alumno.usuarioId">
